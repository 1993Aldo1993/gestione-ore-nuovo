...
export default function Home() {
  ...
  const caricaDati = async () => {
    try {
      ...
      if (err1 || err2 || err3) console.error("Errore Supabase:", err1 || err2 || err3)
  ...
  const modificaOre = async (id: number, oreAttuali: number, variazione: number) => {
    ...
    if (nuoveOre < 0) return;

    const { error } = await supabase.from('diario').update({ minerale: nuoveOre }).eq('id', id);
  ...
  // LOGICA FILTRO CORRETTA: converte tutto in minuscolo per il confronto
  const datiFiltrati = records.filter(r => {
    const matchMese = filtroMese === '' || r.mese?.toLowerCase() === filtroMese.toLowerCase()
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="space-y-1">
            ...
            <select className="w-full bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-700 outline-none" onChange={(e) => setFiltroDipendente(e.target.value)}>
          ...
          <div className="space-y-1">
            ...
            <select className="w-full bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-700 outline-none" value={filtroCantiere} onChange={(e) => setFiltroCantiere(e.target.value)}>
          ...
          <div className="space-y-1">
            ...
            <select className="w-full bg-slate-50 p-3 rounded-xl border border-slate-100 text-slate-700 outline-none" onChange={(e) => setFiltroMese(e.target.value)}>
                ...
                <tr key={r.id} className="hover:bg-slate-50/80 transition-all group">
                  ...
                  <td className="p-6 text-center">
                    <div className="flex items-center justify-center gap-3">
                      <button onClick={() => modificaOre(r.id, Number(r.minerale), -1)} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center hover:bg-red-50 hover:text-red-600 transition-colors text-lg font-bold">-</button>
                      <span className="bg-blue-50 text-blue-700 px-4 py-1 rounded-lg font-black text-lg min-w-[50px]">{r.minerale}</span>
                      <button onClick={() => modificaOre(r.id, Number(r.minerale), 1)} className="w-8 h-8 rounded-full border border-slate-200 flex items-center justify-center hover:bg-green-50 hover:text-green-600 transition-colors text-lg font-bold">+</button>
                    </div>
                  </td>
                  <td className="p-6 text-right">
                    <button onClick={() => eliminaRecord(r.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-slate-300 hover:text-red-500 p-2">🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
