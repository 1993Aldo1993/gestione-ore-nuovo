...
export default function RiepilogoOre() {
  ...
  const caricaDati = async () => {
    ...
    const { data: can } = await supabase.from('cantieri').select('*')
  ...
  const totaleOre = datiFiltrati.reduce((acc, curr) => acc + (curr.minerale || 0), 0)
    ...
    <main className="min-h-screen bg-slate-50 p-4 md:p-8 pb-32">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-900 italic">Riepilogo Ore</h1>
            <p className="text-slate-500 text-sm">Registro attività professionale</p>
          </div>
          <div className="bg-white px-8 py-4 rounded-3xl shadow-sm border border-slate-100 text-center w-full md:w-auto">
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Totale Accumulato</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Filtra per Persona</label>
            <select value={filtroPersona} onChange={e => setFiltroPersona(e.target.value)} className="w-full bg-slate-50 p-3 rounded-xl font-bold text-slate-700 outline-none">
              <option>Tutti i Dipendenti</option>
              {dipendenti.map(d => <option key={d.id} value={d.nome_dipendente}>{d.nome_dipendente}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Filtra per Cantiere</label>
            <select value={filtroCantiere} onChange={e => setFiltroCantiere(e.target.value)} className="w-full bg-slate-50 p-3 rounded-xl font-bold text-slate-700 outline-none">
              <option>Tutti i Cantieri</option>
              {cantieri.map(c => <option key={c.id} value={c.nome_cantiere}>{c.nome_cantiere}</option>)}
            </select>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Filtra per Mese</label>
            <select value={filtroMese} onChange={e => setFiltroMese(e.target.value)} className="w-full bg-slate-50 p-3 rounded-xl font-bold text-slate-700 outline-none">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="overflow-x-auto">
            <div className="min-w-[700px]"> {/* Forza larghezza per evitare sovrapposizioni su mobile */}
                ...
                <tbody className="divide-y divide-slate-50">
                  {datiFiltrati.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-6 py-4 text-xs font-medium text-slate-500">{new Date(r.dati).toLocaleDateString('it-IT')}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        {/* Spazio per eventuali altre azioni */}
                      </td>
                    </tr>
                  ))}
                </tbody>
              ...
              {datiFiltrati.length === 0 && (
                <div className="py-20 text-center text-slate-400 font-bold uppercase text-xs tracking-widest">
                  Nessun dato trovato per questi filtri
                </div>
              )}
            </div>
          </div>
